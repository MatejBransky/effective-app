...
// A fresh registry per test - registerKeybinding/dispatchKeydown require
// AtomRegistryService rather than reaching for a shared module-level instance.
...
describe("registerKeybinding/dispatchKeydown", () => {
  it("calls the handler for a matching keypress", () => {
    stubPlatform("Win32");
    const handler = vi.fn();
    const unregister = run(registerKeybinding({ keys: "mod+k", handler }));

    run(dispatchKeydown(event({ key: "k", ctrlKey: true }) as KeyboardEvent));
  ...
  it("does not call the handler for a non-matching keypress", () => {
    ...
    run(dispatchKeydown(event({ key: "j", ctrlKey: true }) as KeyboardEvent));
