...
const closeWith = (registry: AtomRegistry.AtomRegistry, id: string): void => {
 ...
 * isolated registry in a test. `ModalHandle.close` itself stays a plain synchronous
 * callback - it captures the *same* resolved registry this ran against, not a fresh lookup -
 * so `onClick={handle.close}` keeps working unchanged everywhere it's already used. */
export const openModal = (
  render: (handle: ModalHandle) => ReactNode,
  options?: ModalOpenOptions,
): Effect.Effect<ModalHandle, never, AtomRegistryService> =>
  Effect.gen(function* () {
    const registry = yield* AtomRegistryService;
    const id = crypto.randomUUID();
    const handle: ModalHandle = { id, close: () => closeWith(registry, id) };
...
export const closeModal = (id: string): Effect.Effect<void, never, AtomRegistryService> =>
  Effect.gen(function* () {
    const registry = yield* AtomRegistryService;
    closeWith(registry, id);
  });
