...
// A fresh registry per test - confirm/openModal require AtomRegistryService rather than
// reaching for a shared module-level instance.
let registry: AtomRegistry.AtomRegistry;
let TestLive: Layer.Layer<AtomRegistryService>;

beforeEach(() => {
  registry = AtomRegistry.make();
  TestLive = Layer.succeed(AtomRegistryService, registry);
});
...
const getModalCallbacks = (): { onConfirm: () => void; onCancel: () => void } => {
  ...
  const element = entry!.render({
    id: entry!.id,
    close: () =>
      Effect.runSync(Effect.provideService(closeModal(entry!.id), AtomRegistryService, registry)),
  }) as ReactElement<{
    onConfirm: () => void;
    onCancel: () => void;
  }>;
...
describe("confirm", () => {
  it("opens exactly one modal", () => {
    void Effect.runPromise(confirm({ title: "t", message: "m" }).pipe(Effect.provide(TestLive)));
  ...
  it("resolves true when confirmed", async () => {
    const promise = Effect.runPromise(
      confirm({ title: "t", message: "m" }).pipe(Effect.provide(TestLive)),
    );
